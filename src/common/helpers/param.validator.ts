import SwaggerDataTypes from "../swagger/types/swagger-data.type";
import SwaggerInTypes from "../swagger/types/swagger-in.type";
import { Req } from "../types/api.type";
import { Params } from "../types/params.type";

export default function ParamValidate(req: Req, route: any) {
  const { path, query, body, files } = req;
  const [prefix, method, qpath, callback, parameters] = route;
  ...
  const parseMultipartArray = (data: any, param: any) => {
    if (param.items && param.items.type == 'object') {
      if (Array.isArray(data)) {
        data.map((e, index) => { data[index] = JSON.parse(e); });
      }
    }
  };
  const multipart = parameters && Object.keys(parameters).find((k) => {
  ...
  const typeCorrection = (data: any, type: string, itemType?: string) => {
    ...
    switch (type) {
      case SwaggerDataTypes.integer: return parseInt(data);
      case SwaggerDataTypes.number: return parseFloat(data);
      case SwaggerDataTypes.boolean: return JSON.parse(data);
      case SwaggerDataTypes.array:
        {
          const result: any[] = Array.isArray(data) ?
            (itemType ? data.map((value: string) => typeCorrection(value, itemType)) : data)
